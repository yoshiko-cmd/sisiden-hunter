...
実機確認の結果、**モデル枠の残りは5**（[STEP 1-1B](./01-current-site-analysis.md)）。
8モデルをそのまま作ることはできないため、以下を適用する。

| モデル | 扱い | 消費枠 |
|---|---|---|
| `DOCUMENTARIES` `ARTICLES` `PEOPLE` `ISSUES` `AREAS` | 通常モデル | 5 |
| `CATEGORIES` `TAGS` `ORGANIZATIONS` | カテゴリー型/タグ型、または下表の代替 | 0 |

タグ型・カテゴリー型が枠を消費しないことは既存プロジェクトの状態から推定したもの。
**新規モデル作成時に種別を選べるかを実機で確認する**こと。
選べない場合は下表の代替を適用する（いずれもPhase 1には影響しない）。
