...
**カテゴリタイプとカスタムタイプは枠を消費しない**（2026-09-21 実機検証済み /
...
カテゴリタイプは ステータス / タイトル / スラッグ / 公開日時 を既定で持つ。
スラッグがあるため独自URLのページを作れるが、
リード文・本文・画像のプロパティを追加できるかはPhase 3で確認する。

### リッチテキストは1モデルに1つだけ（2026-09-21 実機確認）

**STUDIOでは、リッチテキストをプロパティとして追加できない。**
プロパティタイプの選択肢は以下のみ。
...
写真や引用を最も使う箇所だから（[STEP 6-5](./06-wireframes.md)）。

#### デザインへの影響は小さい

作品ページの「この物語について」「背景」、課題ページの「背景」は、
[STEP 9](./09-design-system.md) のとおり**本文幅680px・明朝・段落のみ**で組む設計。
装飾を前提にしていないため、テキストプロパティで支障はない。

ただし `ARTICLES` の本文だけは、引用ブロック・解説BOX・写真キャプションが必須。
**記事の本文は必ず本文エリアを使う**こと。
