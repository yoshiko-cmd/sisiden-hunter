...
**プランは Business で確認済み。** 上限は CMSモデル10 / CMSアイテム5,000 / 公開300ページ。
Data Connect API・カスタムコード・構造化データ設定はすべて使える。

残る確認は CMS の使用状況のみ。

```
STUDIOエディタを開く > 左サイドバーの [CMS] アイコンをクリック
  → モデルが1つも表示されない  = CMS未使用。10枠すべて使える（本設計は8枠）
  → モデルが表示される         = 名前と数を記録する
```
...
判定:
- **残枠8以上** → 本設計をそのまま実行
- **残枠8未満** → [STEP 5-8 縮退案](./05-studio-cms-model.md) を採用（最小5モデルまで圧縮できる）
